...
FLAGS="-XX:+UseG1GC -XX:G1HeapRegionSize=1m -XX:ParallelGCThreads=${PARALLEL_GC_THREADS} \
		...
		-XX:InitialTenuringThreshold=5 -XX:MaxTenuringThreshold=7 -XX:-AlwaysTenure"


# EXEC=("HashMap" "Array" "Array_List" "Array_List_Int" "List_Large" "MultiList" \
...
EXEC=("MultiHashMap" "Test_WeakHashMap" "ClassInstance")
...
function interpreter_mode() {
	time ${JAVA} ${FLAGS} \
		...
		-Xmx${MAX}g \
		-Xms${XMS}g \
		-XX:ErrorFile=out/$1_hs_err_th${PARALLEL_GC_THREADS}.log \
		-cp ./bin $1 > out/$1_out_th${PARALLEL_GC_THREADS} 2>&1
