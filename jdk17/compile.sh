...
function build_jvm_image() {
  ...
  CC=${CC} CXX=${CXX} bash ./configure \
    ...
    --disable-cds \
    --with-extra-cflags="-march=${microarchitecture} " \
    --with-extra-cxxflags="-march=${microarchitecture} "

  intercept-build make CONF=linux-$TARGET_PLATFORM-server-$image_variant images
  cd ../
  compdb -p jdk17 list >compile_commands_$image_variant.json
  mv compile_commands_$image_variant.json jdk17
  cd - || exit
}
