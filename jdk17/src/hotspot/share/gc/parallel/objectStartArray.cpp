...
#ifdef TERA_CARDS
void ObjectStartArray::th_initialize(MemRegion reserved_region) {
  // We're based on the assumption that we use the same
  // size blocks as the card table.
  assert((int)th_block_size == (int)CardTable::th_card_size, "Sanity");
  assert((int)th_block_size <= (int)CardTable::th_card_size,
		  "block_size must be less than or equal to %d", (1 << CardTable::th_card_shift));

  // Calculate how much space must be reserved
  _reserved_region = reserved_region;

  if (EnableTeraHeap) {
	  DEBUG_ONLY(fprintf(stderr, "==============DEBUGGING====================\n"));
	  DEBUG_ONLY(fprintf(stderr, "Reserved Region: Start %p | End %p \n", reserved_region.start(), reserved_region.end()));
  }

  size_t bytes_to_reserve = reserved_region.word_size() / th_block_size_in_words * sizeof(int);
  assert(bytes_to_reserve > 0, "Sanity");

  if (EnableTeraHeap) {
	  DEBUG_ONLY(fprintf(stderr, "==============DEBUGGING====================\n"));
	  DEBUG_ONLY(fprintf(stderr, "Bytes to Reserved: Bytes B %lu\n", bytes_to_reserve));
  }
  ...
  _th_raw_base = (int*)_virtual_space.low_boundary();
  if (_th_raw_base == NULL) {
    vm_exit_during_initialization("Could not get raw_base address");
  }
  
  if (EnableTeraHeap) {
	  DEBUG_ONLY(fprintf(stderr, "==============DEBUGGING====================\n"));
	  DEBUG_ONLY(fprintf(stderr, "Raw_base %p\n", _th_raw_base));
	  DEBUG_ONLY(fprintf(stderr, "Virtual Space: Low %p | High %p\n", _virtual_space.low_boundary(), _virtual_space.high_boundary()));
	  DEBUG_ONLY(fprintf(stderr, "===========================================\n"));
  }

  MemTracker::record_virtual_memory_type((address)_th_raw_base, mtGC);

  _th_offset_base = _th_raw_base - (size_t(reserved_region.start()) >> th_block_shift);

  _covered_region.set_start(reserved_region.start());
  _covered_region.set_word_size(0);

  _blocks_region.set_start((HeapWord*)_th_raw_base);
  _blocks_region.set_word_size(0);
}


void ObjectStartArray::th_set_covered_region(MemRegion mr) {
  ...
  assert(th_block_for_addr(low_bound) == &_th_raw_base[0], "Checking start of map");
  assert(th_block_for_addr(high_bound-1) <= &_th_raw_base[_blocks_region.byte_size()-1], "Checking end of map");
}


bool ObjectStartArray::th_object_starts_in_range(HeapWord* start_addr,
	...
	int* start_block = th_block_for_addr(start_addr);
	int* end_block = th_block_for_addr(end_addr);
