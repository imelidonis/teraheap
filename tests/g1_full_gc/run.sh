...
function interpreter_mode() {
	${JAVA} ${FLAGS} -XX:+EnableTeraHeap -XX:TeraHeapSize=${TERACACHE_SIZE} -XX:TeraStripeSize=${STRIPE_SIZE} \
		...
		-XX:ErrorFile=./${EXEC_DIR_NAME}/out/$1_hs_err.log -cp ./${EXEC_DIR_NAME}/bin $1  > ./${EXEC_DIR_NAME}/out/$1_out 2>&1
}
...
function run_tests_debug() {
	gdb --args ${JAVA} ${FLAGS} \
		...
		-XX:ErrorFile=./${EXEC_DIR_NAME}/out/$1_hs_err.log -cp ./${EXEC_DIR_NAME}/bin $1
}

mkdir ${EXEC_DIR_NAME}/out
